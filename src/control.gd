...
func draw_loaded_tiles_fixed(center: Vector2):
    ...
    # Access the loaded_tiles through the mutex for thread safety
    terrain_loader.mutex.lock()
    var loaded_tiles = terrain_loader.loaded_tiles.duplicate()
    terrain_loader.mutex.unlock()
...
func get_meters_per_pixel() -> float:
    # Approximate conversion: at zoom 15, a tile is about 500m, and we display it as max_tile_display_size pixels
    # So meters per pixel = (tile size in meters) / (tile size in pixels)
    var base_tile_size_meters = 40000000.0 / pow(2.0, terrain_loader.current_zoom)
...
func draw_debug_info():
    ...
    if terrain_loader:
        var tile_text = "Current Tile: %s at z%d" % [terrain_loader.current_tile_coords, terrain_loader.current_zoom]
        draw_string(font, Vector2(10, y_offset), tile_text)
        y_offset += 20

        # Loaded tiles count
        terrain_loader.mutex.lock()
        var loaded_count = terrain_loader.loaded_tiles.size()
        var queue_count = terrain_loader.download_queue.size()
        terrain_loader.mutex.unlock()

        var loaded_text = "Loaded Tiles: %d" % loaded_count
        draw_string(font, Vector2(10, y_offset), loaded_text)
        y_offset += 20
        ...
        # Cache info
        var cache_text = "Tile Cache: %d/%d" % [terrain_loader.tile_cache.size(), terrain_loader.cache_size]
        ...
        var dynamic_zoom = terrain_loader.calculate_dynamic_zoom(altitude)
        var zoom_text = "Altitude: %.1fm -> z%d" % [altitude, dynamic_zoom]
