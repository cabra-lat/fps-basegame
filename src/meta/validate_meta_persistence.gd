...
func _scenario_hub_deploy_contract() -> void:
	...
	# A successful preparation with auto-insurance disabled must replace, not
	# retain, a stale manifest. A later loss therefore has no insurance effect.
	var stale_item := ItemCodec.item_from_path("res://resources/weapons/M4_Carbine.tres")
	var stale_eq := Equipment.new()
	stale_eq.equip(stale_item, "primary")
	var stale_manifest := ItemCodec.encode_equipment(stale_eq)
	var uninsured_service := MetaService.new()
	var uninsured_profile := MetaProfile.new()
	uninsured_service.use_profile(uninsured_profile, TEST_DIR + "/uninsured_prepare.save")
	uninsured_service.bind_carrier(Equipment.new(), Backpack.new())
	uninsured_service.auto_insure = false
	uninsured_service._insured_manifest = stale_manifest.duplicate(true)
	var uninsured_prepare := uninsured_service.prepare_raid_checked()
	_check(uninsured_prepare.get("ok", false), "checked prepare succeeds without auto-insurance")
	_check(uninsured_service._insured_manifest.is_empty(), "successful auto_insure=false prepare clears stale insurance manifest")
	var uninsured_loss := uninsured_service.resolve_raid(Raid.Outcome.KIA, 0)
	_check(uninsured_loss != null and uninsured_loss.insured == 0 and uninsured_profile.insurance.pending_count() == 0, "uninsured prepare leaves later loss settlement unclaimed")

	# The unconfigured early return is also a new failed attempt: stale insurance
	# state must not survive into a later loss resolution.
	var unconfigured_service := MetaService.new()
	var unconfigured_profile := MetaProfile.new()
	unconfigured_service.use_profile(unconfigured_profile, TEST_DIR + "/unconfigured_prepare.save")
	unconfigured_service._insured_manifest = stale_manifest.duplicate(true)
	var unconfigured_prepare := unconfigured_service.prepare_raid_checked()
	_check(not unconfigured_prepare.get("ok", false), "unconfigured checked prepare reports failure")
	_check(unconfigured_service._insured_manifest.is_empty(), "unconfigured prepare clears stale insurance manifest")
	unconfigured_service.bind_carrier(Equipment.new(), Backpack.new())
	var unconfigured_loss := unconfigured_service.resolve_raid(Raid.Outcome.KIA, 0)
	_check(unconfigured_loss != null and unconfigured_loss.insured == 0 and unconfigured_profile.insurance.pending_count() == 0, "unconfigured prepare leaves later loss settlement unclaimed")
